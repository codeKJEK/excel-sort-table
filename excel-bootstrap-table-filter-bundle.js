(function ($$1) {
  ...
  var FilterMenu = (function () {
    ...
    FilterMenu.prototype.initialize = function () {
      ...
      var headerContainerDiv = $(this.th).find("div")[0]; // UPDATED BY KURTIS KOVEL 11-06-2022
      headerContainerDiv.appendChild(this.menu); // UPDATED BY KURTIS KOVEL 11-06-2022
  ...
  var FilterCollection = (function () {
    ...
    FilterCollection.prototype.bindCheckboxes = function () {
      ...
      this.target.find(".dropdown-filter-menu-item.item").change(function () {
        ...
        changeFilter(); // ADDED BY KURTIS KOVEL 11-06-2022
      });
    };
    ...
    FilterCollection.prototype.bindSort = function () {
      ...
      this.target.find(".dropdown-filter-sort").click(function () {
        ...
        updateRowVisibility(filterMenus, rows, ths);
        updateSortIcon(column, order); // ADDED BY KURTIS KOVEL 11-06-2022
      });
    };
    ...
    FilterCollection.prototype.updateRowVisibility = function (
      ...
      updateRowParity(); // ADDED BY KURTIS KOVEL 11-06-2022
    };
...
// CUSTOM -- KURTIS KOVEL -- 11/04/2022

function updateRowParity() {
  var tableBodyRows = $("tbody tr");
  clearRowParity(tableBodyRows);
  setRowParity(tableBodyRows);
}

function clearRowParity(tableBodyRows) {
  tableBodyRows.removeClass("even odd");
}

function setRowParity(tableBodyRows) {
  var parity = "even";
  tableBodyRows.each(function () {
    var currentRow = $(this),
      currentRowCssDisplay = currentRow.css("display");
    if (currentRowCssDisplay != "none" && parity === "even") {
      currentRow.addClass("even");
      parity = "odd";
    } else if (currentRowCssDisplay != "none" && parity === "odd") {
...
function updateSortIcon(column, order) {
  var sortedColumnHeader = $("th")[column],
    columnHeaders = $("th");
  columnHeaders.removeAttr("data-sort");
  $(sortedColumnHeader).attr("data-sort", order);
  changeFilter();
}

function changeFilter() {
  var currentOrder = checkOrder();
  checkSelectAll(currentOrder);
}
...
function checkOrder() {
  ...
  tableHeaders.each(function () {
    var headerSortAttribute = $(this).attr("data-sort");
...
function checkSelectAll(currentOrder) {
  var allSelectAllCheckboxes = $(".select-all");
  allSelectAllCheckboxes.each(function () {
    var currentCheckbox = $(this),
      filteredColumnNumber = $(this).attr("data-column"),
      filteredColumnHeader = $("th")[filteredColumnNumber];
    if (currentCheckbox.prop("checked") === false) {
      $(filteredColumnHeader).attr("data-filter", currentOrder);
    } else if (currentCheckbox.prop("checked") === true) {
      $(filteredColumnHeader).removeAttr("data-filter");
    }
  });
}
